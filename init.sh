...
if [ -f "$ENV_FILE" ]; then
    ...
    sed -i "s|DATABASE_PASSWORD = 'FastPHMLite3'|DATABASE_PASSWORD = '$MYSQL_PASSWORD'|g" ./backend/conf/env.py
    sed -i "s|REDIS_PASSWORD = 'FastPHMLite3'|REDIS_PASSWORD = '$REDIS_PASSWORD'|g" ./backend/conf/env.py
    echo "初始化密码创建成功"
fi

docker-compose up -d
docker exec fastphmlite3-django python manage.py makemigrations
docker exec fastphmlite3-django python manage.py migrate
docker exec fastphmlite3-django python manage.py init
echo "欢迎使用fastphmlite3项目"
